...
| 告警通知链路 | 本地 webhook 收集器（`notify.json` → 127.0.0.1:19999）+ 临时服务条目制造红灯 | 审批挂起即收 `agentbd 审批等待 · fake · high风险 · $ echo …`；服务红灯**三条路径全部实收**：新增即挂（`red3-test-tmp（新增）· L1 59997 未监听`）、绿→红跃迁（`red-test-tmp · L1 59998 未监听`）、红→绿恢复（`red3-test-tmp 红灯转绿`）；系统通知同时弹出 |
